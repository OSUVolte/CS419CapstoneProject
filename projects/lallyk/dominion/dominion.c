...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	{
		case adventurer:
			return refactorAdventurer(currentPlayer, state);
		...
		case smithy:
			return refactorSmithy(currentPlayer, state, handPos);
		...
		case great_hall:
			return refactorGreatHall(currentPlayer, state, handPos);

		case minion:
			return refactorMinion(choice1, choice2, currentPlayer, state, handPos);
...
/* ********* Refactored Methods: Smithy, Adventurer, Gardens, Great Hall and Minion ********* */

int refactorSmithy(int currentPlayer, struct gameState *state, int handPos){
	int i;
...
int refactorAdventurer(int currentPlayer, struct gameState *state){
	// include the following variable for use in function
...
int refactorGardens(struct gameState *state){
	state->coins = state->coins + 2;
	return -1;   
}
...
int refactorMinion(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos){
	int i;
	int j;
	//+1 action
	state->numActions++;
