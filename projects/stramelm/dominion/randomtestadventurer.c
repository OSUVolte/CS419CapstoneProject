...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < 100000; i++) { // different sets of input conds to test
    ...
    // random player number for this test
    ...
    // run the test
    r = adventurerCardEffect(pn, state);
    calls++;
    ...
    // BUT ADVENTURER WILL BE ADDED BACK TO THE DISCARD, NET -1
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // REQT #7: BOTH CARDS REC'VD BY CURRENT PLAYER SHOULD BE TREASURE CARDS
    // -------------------------------------------------------------------------
    before = 0;
    after = 0;
    for (j = 0; j < copy->handCount[pn]; j++) {
      card = copy->hand[pn][j];
      if (card >= 4 && card <= 6) {
        before++;
      }
    }
    for (j = 0; j < state->handCount[pn]; j++) {
      card = state->hand[pn][j];
      if (card >= 4 && card <= 6) {
        after++;
      }
    }
