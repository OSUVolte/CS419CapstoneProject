...
int length = 27;
int card_array[27] = {curse, estate, duchy, province, copper, silver, gold, 
    adventurer, council_room, feast, gardens, mine, remodel, smithy, village, baron,
    great_hall, minion, steward, tribute, ambassador, cutpurse, embargo, outpost, salvager,
    sea_hag, treasure_map};
int value_array[27] = {0, 2, 5, 8, 0, 3, 6, 6, 5, 4, 4, 5, 4, 4, 3, 4, 3, 5, 3, 5, 3, 4, 2, 5, 4, 4, 4};
int i;
int passed = 0;
for(i = 0; i< length; i++){
    int cost = getCost(card_array[i]);
    if(cost != value_array[i]){
        printf("%d: FAIL value should be %d but is %d\n", i, value_array[i], cost);
    } 
    else
        passed++;
}
