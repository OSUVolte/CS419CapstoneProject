...
void printCurGameState(struct gameState *G){
	...
	printf("Supply count for players: ");
	for(int i = 0; i < treasure_map + 1; i++){
		printf("%i ", G->supplyCount[i]);
		if(i % 10 == 0 && i > 2 && i < 12) printf("\n");
	}
	printf("\n");
...
void printResults(struct gameState *G, struct gameState *I){
	...
	printCurGameState(I);	
	for(int i = 0; i < treasure_map + 1; i++){
		if(I->supplyCount[i] != G->supplyCount[i]){
			printf("Unmatched card: %i ", i);
			printf("Card amounts: %i and %i\n", I->supplyCount[i], G->supplyCount[i]);
		}
	}
}
...
struct compareReturn compareGameStates(struct gameState *left, struct gameState *right, int player){
	...
	for(int i = 0; i < treasure_map + 1; i++){
		if(left->supplyCount[i] != right->supplyCount[i]){
			retArr.supply++;
		}
	}	
...
int printCompareResults(struct compareReturn results){
	...
	if(results.supply){
		printf("***Supply count does not match.\n");
		noProblem = 0;
	}
	if(!noProblem) printf("\n");	
	return noProblem;
