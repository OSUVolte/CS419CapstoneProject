...
int playGreatHall(struct gameState *state, int handPos){
      int currentPlayer = whoseTurn(state);
      handPos = state->hand[currentPlayer][handPos];
      //+1 Card
      drawCard(currentPlayer, state);
...
int playAmbassador(struct gameState *state, int handPos, int choice1, int choice2){
  ...
  int i;
      ...
      if (choice2 > 2 || choice2 < 0)//cannot choose more than 2
  {
    return -1;        
  }

      if (choice1 == handPos)//cant choose this ambassador
  {
    return -1;
  }
  ...
  printf("Player %d reveals card number: %d\n", currentPlayer, state->hand[currentPlayer][choice1]);
      ...
      state->supplyCount[choice2] += choice2;
