...
int main() {
	...
	cardMods.card = village;
	cardMods.nActions = 2;  /* Number of additional actions */
	cardMods.nBuys = 0;  /* Number of additional buys */
	cardMods.nCards = 1;  /* Number of additional cards to hand */
	...
	for (n = 0; n < NUM_TESTS; n++) {
		...
		if (TEST_DEBUG) {
			printf("\nCurrent Player's hand before adding %s card ...\n", TEST_CARD);
		...
		if (TEST_DEBUG) {
			printf("\nCurrent Player's hand after adding %s card ...\n", TEST_CARD);
			_printValidHand(&testGame, curPlayer);
		}
