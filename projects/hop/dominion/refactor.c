...
Details:
   ...
   playSmithy()
      - Introduced a bug by adding a ";" at the end of the card drawing for loop.  This will only call drawCard() once.
   ...
   playFeast()
      - The for loop control is changed from "for (i=0; i <= state->handCount[currentPlayer]; i++)" to "for(i=0; i < state->handCount[currentPlayer]; ++i)".  The change from post-increment to pre-increment is a cosmetic change and is benign. However, the end loop condition change from "<=" to "<" is a bug.
   ...
   playCouncil_Room()
      ...
      - Removed call to "discardCard()" entirely. This is an introduced bug.
