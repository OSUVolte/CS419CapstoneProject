 ...
 * CS 362 Assn 3
 * cardtest4.c
 * Test of Garden card in cardEffect() in  dominion.c
...
int main() {
    ...
    int numPlayer = 2;
    int r, i;
    int k[10] = {gardens, adventurer, salvager, embargo, ambassador, steward,
    ...
    //Test Garden
    //Make sure nothing happens except Garden moves piles
    ...
    cardEffect(gardens, choice1, choice2, choice3, &G, handpos, &bonus);
    ...
    //Check to make sure garden not in hand
    //Note this test only works if there was only 1 garden in the hand
    ...
    if (gardenInHand == 1)
        ...
        theResult(1, &testsPassed, &testsCompleted);
    printf("Garden in hand:\tExpected 0, got %d\n", gardenInHand);
    ...
    if (G.discard[1][0] == gardens && G.discard[1][1] == feast)
        theResult(1, &testsPassed, &testsCompleted);
    else
        theResult(0, &testsPassed, &testsCompleted);
    ...
    if (origG.supplyCount[gardens] == G.supplyCount[gardens])
        theResult(1, &testsPassed, &testsCompleted);
    else
        theResult(0, &testsPassed, &testsCompleted);
    printf("Number of gardens cards should not change.\n");
    ...
    printf("----------FINISHED TESTING gardens CARD----------:\n\n");
