/* -----------------------------------------------------------------------
		Unit Testing for village card 
		**Used testUpdateCoins.c template for r

		___ Village ___
			Basic Requirements:
				1. Current Player Should Recieve Exactly 1 Cards and 2 Actions
...
int main() {
	...
	// ----------- TEST 3: Village is discarded from hand--------------
	failFlag = 0;
	printf("TEST 3: Village is successfully discarded, Other Cards are kept\n");
	...
	for (i=0; i<G.handCount[thisPlayer]; i++) {
			...
			G.hand[thisPlayer][i] = Village;			 // 0 1 2 3 4 
			...
			printf("Cards in Hand Before Playing Village: ");
			...
			for (m=0; m<G.handCount[thisPlayer]; m++) {
				if( m=i ){
					if( G.hand[thisPlayer][m] == testG.hand[thisPlayer][m] ){
						printf( "Test 3 Failed: Village remained in same position in hand for iteration # %d, where Village was in starting position: %d.\n", i+1, i );
						failFlag = 1;
					} else if ( testG.hand[thisPlayer][m] == smithy ){
						printf( "Test 3 Failed: Village remained in hand for iteration # %d, but moved to position %d, where Village was in starting position: %d.\n", i+1, m, i );
						failFlag = 1;
					} else {
						passCount++;
	...
	// ----------- TEST 4: Coin, Buy, Kingdom, Victory states don't change ---------
	failFlag = 0;
	handpos = 0;
	printf("TEST 4: Coin, Buy, Kingdom, Victory states don't change\n");
