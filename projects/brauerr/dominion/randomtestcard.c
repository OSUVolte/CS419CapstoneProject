...
int initializeRandomGame(struct gameState *state) {
  ...
  state->deck[0][state->deckCount[0]] = smithy;
  state->deckCount[0]++;
...
int main() {
  ...
  int i, j;
  int decrementDeckPass = 0;
  int incrementHandPass = 0;
  ...
  printf("Testing Smithy %d times...\n", numberRandomRuns);
  printf("Decrement Deck Passed %d times\n", decrementDeckPass);
  printf("Increment Hand Passed %d times\n", incrementHandPass);
