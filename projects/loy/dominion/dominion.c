...
if(card == smithy)
{
for (i = 0; i < 4; i++) //changed to 4 instead of 3
	{
...
if(card == adventurer)
{
while(drawntreasure<=2){  // changed to <= 2 instead of < 2
...
{
for (i = 0; i < 3; i++) // changed to 3 instead of 4
	{
	  drawCard(currentPlayer, state);
	}
...
for (i = 0; i <= state->handCount[currentPlayer]; i++){
	...
	state->hand[currentPlayer][i] = -1;//Set to nothing
      ...
      updateCoins(currentPlayer, state, 6); // changed to 6 instead of 5
