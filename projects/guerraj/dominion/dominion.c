...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
       return callAdventureCard(temphand, state, z, cardDrawn, drawntreasure, currentPlayer);
		
    case council_room:
      return callCouncilRoom(currentPlayer, state, handPos);
    ...
    case smithy:
      return callSmithyCard(currentPlayer, state, handPos);
	  
    case village:
	return callVillageCard(currentPlayer, state, handPos);
    ...
    case great_hall:
      return callGreatHall(currentPlayer, state, handPos);
...
int callAdventureCard(int hand[], struct gameState* state, int z, int cardDrawn, int drawntreasure, int currentPlayer)
{
	...
	if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
      ...
      while(z-1>=0){
	state->discard[currentPlayer][state->discardCount[currentPlayer]++]=hand[z-1]; // discard all cards in play that have been drawn
	z=z-1;
      }
      return 0;
