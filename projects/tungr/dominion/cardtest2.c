...
int main(){
	...
	if(G.handCount[currentPlayer] == baseHandCount + 1){ //take in 2 cards but put one back
		printf("	PASS\n"); 
		printf("		Player received just 2 cards\n");
		printf("		current handCount: %d, previous handCount: %d\n", G.handCount[currentPlayer], baseHandCount);

	} else {
		printf("	FAIL\n");
		printf("		Player did not receive just 2 cards\n");
		printf("		current handCount: %d, previous handCount: %d\n", G.handCount[currentPlayer], baseHandCount);
	}
	
	//check number of cards given from players deck 
	if(G.deckCount[currentPlayer] == baseDeckCount - 2){ //give 1 card but take 2 in 
		printf("	PASS\n"); 
		printf("		exactly 2 cards taken from currentPlayers Deck\n\n");
		printf("		current deckCount: %d, previous deckCount: %d\n", G.deckCount[currentPlayer], baseDeckCount);		
	} else {
		printf("	FAIL\n");
		printf("		exactly 2 cards not taken from currentPlayers Deck\n");
		printf("		current deckCount: %d, previous deckCount: %d\n", G.deckCount[currentPlayer], baseDeckCount);
