...
int tributeAction(struct gameState* state, int* nextPlayer, int tributeRevealedCards[], int* currentPlayer) {
	int i;
	...
	for (i = 0; i <= 2; i++) {
		if (tributeRevealedCards[i] == copper || tributeRevealedCards[i] == silver || tributeRevealedCards[i] == copper) {//Treasure cards
			state->coins += 2;
		}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case tribute:
		return tributeAction(state, &nextPlayer, tributeRevealedCards, &currentPlayer);
    ...
    case cutpurse:
		...
		return cutpurseAction(&currentPlayer, state, &handPos);
