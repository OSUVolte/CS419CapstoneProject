...
int basicReturnTest(struct gameState* state){
   ...
   if(retVal != gameOver){
      printf("basicReturnTest -1: Game Over Conditions Don't Match\n");
      return -1;
   }
   return 0;
}
...
int allNonProvinceEmptyReturnTest(struct gameState* state){
   ...
   if(retVal != checkGameOver(state)){
      printf("allNonProvinceEmptyReturnTest -1: Game Over Evaluation Differs\n");
      return -1;
   }
...
int noEmptySuppliesReturnTest(struct gameState* state){
   ...
   int retVal = isGameOver(state);
   if(retVal != checkGameOver(state)){
      printf("noEmptySuppliesReturnTest -1: Game Over Evaluation Differs\n");
      return -1;
   }
...
int unitTest(){
   ...
   for(i = 2; i <= 4; i++){
	 ...
	 printf("unitTest -5: Failed No Empty Supplies Return Test\n");
	 //return -5;
      } else {
   ...
   for(i = 2; i <= 4; i++){
	 ...
	 testState->supplyCount[j] = 0;
      ...
      if(checkGameOver(testState) != isGameOver(testState)){   
	 printf("specificStateTest -1: Failed Specific State Test\n");
	 //return -6;
      } else {
	 printf("provinceEmptyReturnTest (%i players): Successful\n", i);
      }
