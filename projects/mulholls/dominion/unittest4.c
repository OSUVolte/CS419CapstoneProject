...
int basicReturnTest(struct gameState* state){
   ...
   for(i = 0; i < state->numPlayers; i++){
      ...
      if(retVal){
	 printf("basicReturnTest -1: discardCard Returning Non-zero Value\n");
	 return -1;
      }
...
int stateTest(struct gameState* state){
   ...
   for(i = 0; i < state->numPlayers; i++){
      ...
      retVal = _stateTestDiscreteTest(0,i,state,cpy,0,4);
      if(retVal){
	 printf("stateTest -1: Too Many State Changes\n");
      }
      memcpy(state, cpy, sizeof(struct gameState));

      //Perform test on card in last position of hand, and non-trash card
      retVal = _stateTestDiscreteTest(state->handCount[i]-1,i,state,cpy,0, 4);
      if(retVal){
	 printf("stateTest -1: Too Many State Changes\n"); 
      }
      memcpy(state, cpy, sizeof(struct gameState));
      ...
      retVal = _stateTestDiscreteTest(0,i,state,cpy,1,2);
      if(retVal){
	 printf("stateTest -1: Too Many State Changes\n"); 
      }
      memcpy(state, cpy, sizeof(struct gameState));
      ...
      retVal = _stateTestDiscreteTest(state->handCount[i]-1,i,state,cpy,1,2);
      if(retVal){
	 printf("stateTest -1: Too Many State Changes\n"); 
      }
      memcpy(state, cpy, sizeof(struct gameState));
