...
int cardEffectAdventurer(int player, int handPos, struct gameState *state)
{
	int drawntreasure = 0;
	int cardDrawn;
	int z = 0;
...
int cardEffectCouncilRoom(int player, int handPos, struct gameState *state)
{
	int i;
...
int cardEffectFeast(int player, int handPos, int choice1, struct gameState *state)
{
	int i;
	int x;
	int temphand[MAX_HAND];// moved above the if statement
...
//Backup hand
	for (i = 0; i <= state->handCount[player]; i++) {
		temphand[i] = state->hand[player][i];//Backup card
		state->hand[player][i] = -1;//Set to nothing
	}
	//Backup hand
	...
	//Reset Hand
	for (i = 0; i <= state->handCount[player]; i++) {
		state->hand[player][i] = temphand[i];
		temphand[i] = -1;
	}
	//Reset Hand
...
int cardEffectMine(int player, int handPos, int choice1, int choice2, struct gameState *state)
{
	int i, j;

	j = state->hand[player][choice1];  //store card we will trash

	if (state->hand[player][choice1] < copper || state->hand[player][choice1] > gold)
	{
		return -1;
	}
	...
	gainCard(choice2, state, 2, player);
...
int cardEffectRemodel(int player, int handPos, int choice1, int choice2, struct gameState *state)
{
	int i, j;
...
int cardEffectSmithy(int player, int handPos, struct gameState *state)
{
	int i;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	{
	case adventurer:
		return cardEffectAdventurer(currentPlayer, handPos, state);

	case council_room:
		return cardEffectCouncilRoom(currentPlayer, handPos, state);

	case feast:
		return cardEffectFeast(currentPlayer, handPos, choice1, state);

	case gardens:
		return -1;

	case mine:
		return cardEffectMine(currentPlayer, handPos, choice1, choice2, state);

	case remodel:
		return cardEffectRemodel(currentPlayer, handPos, choice1, choice2, state);

	case smithy:
		return cardEffectSmithy(currentPlayer, handPos, state);
