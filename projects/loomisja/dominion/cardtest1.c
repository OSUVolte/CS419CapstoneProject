...
**		Unit test for the Smithy card effect.
** Rule: +3 Cards
...
** 2. Current player receives N = 3 cards
...
void testStandardConditions(int players, int player, int handpos, struct gameState* pre, struct gameState* post, int* passes, int* failures)
{
	...
	/* 2. Current player receives 3 cards */
	...
	if (OUTPUTLEVEL > 1) printf("Result: Added %d cards, expected %d. ", result, 3);
	updateTestResult(result == 3, passes, failures);
	result1 = result;

	/* 3. 3 cards received from the current player's deck */
	...
	if (OUTPUTLEVEL > 1) printf("Result: Removed %d cards, expected %d. ", -result, 3); /* deck supply cards should decrease, so check negative of result */
	updateTestResult(-result == 3, passes, failures);
	result2 = -result;
