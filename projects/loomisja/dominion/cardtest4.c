...
**		Unit test for the Village card effect.
** Rule: +1 Card, +2 Actions
...
** 2. Current player receives N = 1 cards
** 3. N cards received came from the current player's deck
** 4. The current player's actions count is increased by 2
** 5. The number and type of the player's set of cards (in his deck, hand,
**		played, discard) are unchanged (no cards "magically" appear or disappear).
** 6. No state changes occur to other players
** 7. No state changes occur to the supply cards (kingdom, treasure, curse)
...
#define CARDUNDERTEST village
#define FILENAME "cardtest4.c"
...
void testStandardConditions(int players, int player, int handpos, struct gameState* pre, struct gameState* post, int* passes, int* failures)
{
	...
	/* 2. Current player receives 1 cards */
	...
	if (OUTPUTLEVEL > 1) printf("Result: Added %d cards, expected %d. ", result, 1);
	updateTestResult(result == 1, passes, failures);

	/* 3. 1 cards received from the current player's deck */
	...
	if (OUTPUTLEVEL > 1) printf("Result: Removed %d cards, expected %d. ", -result, 1); /* deck supply cards should decrease, so check negative of result */
	updateTestResult(-result == 1, passes, failures);

	/* 4. The current player's actions count is increased by 2 */
	if (OUTPUTLEVEL > 1) printf("Test: Current player action counter incremented +2...\n");
	result = post->numActions - pre->numActions;
	if (OUTPUTLEVEL > 1) printf("Result: numActions increased by %d, expected %d. ", result, 2);
	updateTestResult(result == 2, passes, failures);

	/*
	* 5. The cards of the player's set of cards (in deck, hand, played, discard)
	...
	*/
	...
	/* 7. No state changes occur to the supply cards(kingdom, treasure, curse) nor to the victory cards */
