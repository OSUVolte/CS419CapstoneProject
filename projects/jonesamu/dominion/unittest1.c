...
Sam Jones
Date: 04/25/16
Description: buyCard() Unit Test.  
- Test conditions for failed execution.  
- Tests that coin count is decremented after card purchased.  
- Tests top confirmed purchased cards are added to deck.
*/
...
void testBuyCard()
{
	...
	/*Tests when player has no buy card*/
	state.numBuys = 0;
	...
	/*Test when player has one buy card*/
	state.numBuys = 1;
	state.supplyCount[1] = 1;
	state.coins = 10;
	r = buyCard(1, &state);
	...
	/*Test when the player has 100 buys*/
	state.numBuys = 100;
	state.supplyCount[1] = 1;
	state.coins = 10;
	r = buyCard(1, &state);
	...
	/*Test when trying to buy a card when their are no buys left.*/
	state.numBuys = 1;
