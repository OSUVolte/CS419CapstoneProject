...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    if(playAdventurer(state, currentPlayer)){
      return -1;
    }
    ...
    case council_room:
      //+4 Cards
      if (playCouncil_Room(state, handPos, currentPlayer) < 0){
        return -1;
      }
    ...
    if(playSmithy(state, handPos, currentPlayer) < 0)
      {
          return -1;
      } 
    ...
    case village:
      ...
      if(playVillage(state, handPos, nextPlayer) < 0){
    ...
    if(playSalvager(state, currentPlayer, handPos, choice1) > 0){
...
int updateCoins(int player, struct gameState *state, int bonus)
{
  ...
  return 0;
    ...
    //handles playing  smithy for cardeffect
      ...
      for (i = 0; i < 3; i++)
  {
   ...
   //handles playing  adventurer for cardeffect
  ...
  drawntreasure=drawntreasure-1;
      ...
      //handles playing  village for cardeffect
      int playVillage(struct gameState *state, int handPos, int currentPlayer){
      //+1 Card
      drawCard(currentPlayer, state);
      
      //+2 Actions
      state->numActions = state->numActions + handPos;
     ...
     //handles playing  council_room for cardeffect
...
//handles playing  salvager for cardeffect
